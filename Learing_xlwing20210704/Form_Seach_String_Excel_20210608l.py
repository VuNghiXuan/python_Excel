...
def show_form():
    class formExcel(Frame):
        ...
        def placeGui(obj, event):
            ...
            # 7.Entry Seach
            obj.H_entrySeach = 30
            obj.W_entrySeach = (objW-obj.posX_lbTieudeSeach-50)*2/3
            ...
            obj.W_trView = objW-obj.posX_lbTieudeSeach-50
            ...
            # obj.H _lbTieudelistboxSheet = 25
        ...
        def show_data_trView(obj):
            ...
            obj.df = obj.excels_ThisCom[id_f].sheets[id_sh].data # gán lại cho giống Pandas
            
            obj.df_cols = list(obj.df.columns)
            cols = obj.df_cols
            obj.trView["column"] = cols
            obj.inser_Cols_into_trView(cols)

            obj.df_rows = obj.df.to_numpy().tolist()
            #df.to_numpy() tạo mảng >>>array([fdvdfvdf])
            # df.to_numpy().tolist tạo list
            rows = obj.df_rows
        ...
        def seach_string_trView(obj, e):            
            ...
            if in_str != "":
                ...
                if in_str.find(chr_first_Col)!=-1 and in_str.find(chr_after_Col)!=-1:
                    # Lấy tên cột và chuôix cần tìm
                    # ["title  "]=m2,["HangMuc"]="Cắt cỏ"'; # ["title  "]="Boss",["Genres"]="Roman"
                    list_value_Cols, list_value_Rows = find_valueStr_OfRowCol_on_trView(in_str)
                    obj.result_findStr_followCols(list_value_Cols, list_value_Rows)
                else:
                    # Tìm str bất kỳ trên cả bảng(ko ràng buộc hàng hay cột)
                    obj.find_all_Str_on_trView( in_str)
            elif in_str == "": # ko có chuỗi thì show data
                obj.show_data_trView()

        def find_all_Str_on_trView(obj, in_str):
        ...
        def result_findStr_followCols(obj, list_value_Cols, list_value_Rows):
            for col_i in range(len(obj.df_cols)):
                pass
            # for col_i in range(len(obj.df_cols)):
            #     for list_value_Cols_i in list_value_Cols:
            #         col = str(obj.df_cols[col_i]).lower()
            #         list_value_Cols_i = str(list_value_Cols_i).lower()
                    
            #         if col == list_value_Cols_i:
            #             # pass
            #             for row_i in range(len(obj.df_rows)):
            #                 for value_Rows in list_value_Rows:
            #                    value_Rows=str(value_Rows).lower()
            #                    value_cell=str(obj.df_rows[row_i][col_i]).lower()
            #                    if value_cell.find(value_Rows)!=-1:
            #                        obj.inser_Row_into_trView(obj.df_rows[row_i])

                            # for str_i in obj.df_rows[row_i][col_i]:
                            #     # ["title  "]="Boss",["Genres"]="Roman"
                            #       ["title  "]="Love's",["Genres"]="War"
                            #      ["title  "]="Love", ["Country"]="Australia"
                            #     print(str_i)
                            #     # for cell in row:
                            #     #     in_str = str(in_str)
                            #     #     cell = str(cell)
