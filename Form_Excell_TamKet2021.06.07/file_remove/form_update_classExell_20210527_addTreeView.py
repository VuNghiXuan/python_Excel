...
def show_form():
    class formExcel(Frame):
        ...
        def __init__(obj, master):
            ...
            obj.lbTieudeSeach = Label(obj, text="Seach Data:", fg = "blue",font=("Times New Roman", 25))              
            ...
            # 3a.Entry Seach
            obj.vcmd = (obj.register(obj.onValidate),
                '%d', '%i', '%P', '%s', '%S', '%v', '%V', '%W')
            obj.entry_Seach = Entry(obj, validate="key", validatecommand=obj.vcmd, text="Tìm chuỗi", fg = "green", font=("Times New Roman", 15) )
            
            # 3b.TrView
            obj.trView = doimau.Treeview(obj, selectmode ='browse')
            obj.scroll_X_trView = Scrollbar(window, 
                           orient="horizontal", 
                           command = obj.trView.xview)
            obj.scroll_X_trView.configure(command = obj.trView)
            obj.trView.configure(xscrollcommand = obj.scroll_X_trView.set) 

            obj.scroll_Y_trView = Scrollbar(window, 
                           orient ="vertical", 
                           command = obj.trView.yview)
            obj.scroll_Y_trView.configure(command = obj.trView)
            obj.trView.configure(xscrollcommand = obj.scroll_Y_trView.set)    

            # self.entry = tk.Entry(self, validate="key", validatecommand=vcmd)
            # elf.text = tk.Text(self, height=10, width=40)            
