...
def show_form():
    class formExcel(Frame):
        ...
        def show_title_playlists(obj, i_pl):
            ...
            # if hasattr(obj.list_App, f"{obj.playlists[i_pl]}"):
        ...
        def read_lixtboxFile_get_i_lbox(obj):
            ...
            for i_lbox in range(total_id):
                ...
                # if val.find("File on This Computer")!=-1:
            return obj.row_title_playlists, obj.row_title_ThisCom, obj.row_title_OtherCom 
        
        def onSelect_listboxFile(obj, event_click): #trả về giá trị tại vị trí click                      
            # Nhận id từ listboxFile
            obj.row_title_playlists, obj.row_title_ThisCom, obj.row_title_OtherCom = obj.read_lixtboxFile_get_i_lbox()
        ...
        def return_result_after_Click(obj, i_click):
            ...
            # ???/
                ...
                # ?lấy i_pl chỗ này.....vì sau xóa thì chỗ này <>0
                playlist_ThisCom = obj.playlists[0]





                if playlist_ThisCom.click==True:# for excel_ThisCom in playlist_ThisCom:                            
                    playlist_ThisCom.click=False
                    obj.return_default_IsFalse_files(playlist_ThisCom)

                else: 
                    playlist_ThisCom.click=True
                    obj.return_IsTrue_files(playlist_ThisCom)

                    # obj.change_Atribute_click_for_all_file_ThisCom(playlist_ThisCom) # Thay đổi False ==True
                obj.update_listboxFile()
            ...
            # Trường hợp click vào tiêu đề OtherCom #"File on This Computer"
                ...
                # ????? Tìm cách lấy i_pl chỗ này    

                playlist_OtherCom = obj.playlists[1]



               
                if playlist_OtherCom.click==True:# for excel_ThisCom in playlist_OtherCom:                            
                    playlist_OtherCom.click=False
                    obj.return_default_IsFalse_files(playlist_OtherCom)

                else: 
                    playlist_OtherCom.click=True
                    obj.return_IsTrue_files(playlist_OtherCom)
            ...
            #click ngoài các tiêu đề App, ThisCom, OtherCom
                ...
                if obj.idx_ClickOnListboxFile[i_click] < obj.row_title_OtherCom: # phạm vi ThisCom
                    ...
                    # for id_click in len((obj.idx_ClickOnListboxFile[i_click])):
        ...
        def showOrhide_playlist(obj):
            for playlist in obj.playlists:
                if playlist.click==True:
                    playlist.click=False
            for playlist in obj.playlists:
                if playlist.click==False:
                    playlist.click=True
                else: 
                    playlist.click=False
            # obj.update_listboxFile()
        ...
        def find_file_sheet_data_After_Clicked(obj, In_str, id_click):
            ...
            if str!="":
                if str == 'Cols':# Đoạn này chờ có sử dụng gì row, col ko????/
                    # id_sheet = id_click-3
                    ...
                    pass
                elif str == "Rows": #Đoạn này chờ có sử dụng gì row, col ko????/
                    # id_sheet = id_click-2
                    # obj.id_sheet_classExell = obj.find_sheet_After_Clicked(id_sheet)                    
                    # obj.id_file_classExell = obj.find_file_After_Clicked(id_sheet-1)#-1: Thực hiện đếm lùi trên listbox để tìm dòng file
                    pass
                if str == "Data":
                    ...
                    obj.id_sheet_classExell = obj.find_sheet_After_Clicked(id_sheet)                    
                    obj.id_file_classExell = obj.find_file_After_Clicked(id_sheet-1)#-1: Thực hiện đếm lùi trên listbox để tìm dòng file
                    """ *********    Đoạn này sau này xuất data qua listbox 2  *************************"""
                ...
                # sheetname[5:len(sheetname)]
                    ...
                    obj.change_Atribute_click_for_File(id_file)
                    obj.update_listboxFile()
        ...
        def find_file_After_Clicked(obj, id_begin):
            ...
            # if findStr_From_chrSpecial(In_str, "[", "]", left=True) == "Data":
            #     pass # sau này show data qua listbox 2 xử lý sau 
            # else    Str = findStr_From_chrSpecial(In_str, ".", "", left=True)

        def return_default_IsFalse_files(obj, playlist_ThisCom):
            for file in playlist_ThisCom.excels:
                file.click=False

        def return_IsTrue_files(obj, playlist_ThisCom):
            for file in playlist_ThisCom.excels:
                file.click=True
        ...
        def change_Atribute_click_for_File(obj, id_file_classExell):
        ...
        def change_Atribute_click_for_sheet_ThisCom(obj, id_file_classExell, id_sheet_classExell):
            if obj.excels_ThisCom[obj.id_file_classExell].sheets[obj.id_sheet_classExell].click == True:
        ...
        def onClick_remove(obj):
            for id in obj.listboxFile.curselection():
                In_str = obj.listboxFile.get(id)
                if In_str[0:6]=="------":                    
                    del obj.playlists
                    obj.update_listboxFile()
                    ...
                    break 
                    # isdir(obj.list_App)
                    # issubclass(obj.list_App)
                    # isinstance
                
                elif In_str[0:2]=="I.":
                    # t=hasattr(obj.list_App,"playlists")
                    # print("truoc kiem tra", t)
                    if hasattr(obj.list_App, "playlists"):
                        del obj.playlists[0]
                        # obj.listboxFile.delete(obj.row_title_ThisCom, obj.row_title_OtherCom-1)
                        obj.update_listboxFile()
                        break
                    # obj.listboxFile.delete(0, END)#def isClass_Exists(obj)
                    # obj.listboxFile.delete(obj.row_title_ThisCom, obj.row_title_OtherCom-1)
                    # t=hasattr(obj.list_App,"playlists")
                    # print("sau kiem tra", t)
                    
                    
                elif In_str[0:3]=="II.":
                    if hasattr(obj.list_App, "playlists"):
                        del obj.playlists[1]
                        obj.listboxFile.delete(obj.row_title_OtherCom, END)
                        # obj.update_listboxFile()
                        break
                    # obj.listboxFile.delete(obj.row_title_OtherCom, END)
                    break   
